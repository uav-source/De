#!/usr/bin/env python3
"""Persistent fixed-order supervisor for the final Day 5 V5 matrix."""
...
RUNNER_HEARTBEAT_STALE_SEC = 20.0
RUNNER_HEARTBEAT_START_GRACE_SEC = 30.0
SINGLE_RUN_WALL_TIMEOUTS = {
...
class Supervisor:
    ...
    def acquire_lock(self) -> None:
        ...
        try:
            ...
            raise RuntimeError("another V5 supervisor owns the exclusive lock") from error
    ...
    def install_inputs(self) -> tuple[Path, Path]:
        ...
        if external_lock.get("run_id") != RUN_ID:
            raise RuntimeError("run lock does not authorize the fixed V5 run-id")
        ...
        installed_lock = self.run_root / "day5_startup_sync_v5_run_lock.json"
    ...
    def matrix_state(self) -> dict[str, Any]:
        return {
            "schema_version": "day5_startup_sync_v5_matrix_state_v1",
            "run_id": RUN_ID,
            "status": self.status,
    ...
    def write_terminal_evidence(self) -> None:
        source = self.run_root / "matrix_state.json"
        if source.is_file():
            shutil.copy2(source, self.run_root / "supervisor_final_state.json")
        ...
        with (self.run_root / "supervisor_summary.csv").open(
    ...
    def finalize_partial(self, endpoint: Path, run_lock: Path) -> None:
        with (self.run_root / "finalize_v5_partial.log").open(
            "w", encoding="utf-8"
        ) as stream:
            subprocess.run(
                [
                    sys.executable,
                    str(ROOT / "scripts/62_finalize_day5_startup_sync_v5.py"),
                    "--run-root",
                    str(self.run_root),
                    "--endpoint-contract",
                    str(endpoint),
    ...
    def run_child(
        ...
        command = [
            sys.executable,
            str(ROOT / "scripts/60_run_single_startup_sync_v5.py"),
            "--run-id",
            RUN_ID,
            "--sequence-id",
            sequence,
        ...
        heartbeat_path = (
            ...
            / "single_run_heartbeat.json"
        )
        started_ns = time.monotonic_ns()
        timeout_ns = int(SINGLE_RUN_WALL_TIMEOUTS[sequence] * 1e9)
        heartbeat_grace_ns = int(RUNNER_HEARTBEAT_START_GRACE_SEC * 1e9)
        heartbeat_stale_ns = int(RUNNER_HEARTBEAT_STALE_SEC * 1e9)
        forced_failure = ""
        with log.open("w", encoding="utf-8") as stream:
            ...
            while self.current_child.poll() is None:
                ...
                if now_ns - started_ns > timeout_ns:
                    forced_failure = "single-run wall timeout"
                    ...
                    try:
                        ...
                        if (
                            now_ns
                            - int(heartbeat["last_heartbeat_monotonic_ns"])
                            > heartbeat_stale_ns
                        ):
                            forced_failure = "single-run heartbeat stale"
                    except Exception:
                        forced_failure = "single-run heartbeat invalid"
                elif now_ns - started_ns > heartbeat_grace_ns:
                    forced_failure = "single-run heartbeat missing"
                if forced_failure:
                    self.failure_classification = "RUNNER_SESSION_INTERRUPTED"
                    self.failure_message = forced_failure
                    try:
                        os.killpg(self.current_child.pid, signal.SIGTERM)
                    except ProcessLookupError:
                        pass
                ...
                time.sleep(1.0)
            try:
                code = self.current_child.wait(timeout=30)
            except subprocess.TimeoutExpired:
                try:
                    os.killpg(self.current_child.pid, signal.SIGKILL)
                except ProcessLookupError:
                    pass
                code = self.current_child.wait(timeout=5)
        self.current_child = None
        self.heartbeat(force=True)
        if forced_failure and code == 0:
            return 30
        return code
    ...
    def final_compare(self, endpoint: Path, run_lock: Path) -> bool:
        ...
        self.heartbeat(force=True)
        with (self.run_root / "finalize_v5.log").open(
            "w", encoding="utf-8"
        ) as stream:
            completed = subprocess.run(
                [
    ...
    def run(self) -> int:
        ...
        for sequence in SEQUENCES:
            ...
            for repeat in REPEATS:
                ...
                if self.interrupted:
                    ...
                    self.heartbeat(force=True)
                    self.write_terminal_evidence()
                    self.finalize_partial(endpoint, run_lock)
                    return 130
                if code != 0:
                    failure_path = (
                        self.run_root
            ...
            if sequence == "avia_quick_shack" and not self.compare_quick(
                ...
                self.failure_classification = "INPUT_BOUNDARY_NONDETERMINISM"
        ...
        if not passed:
            summary_path = self.run_root / "day5_startup_sync_v5_summary.json"
        ...
        self.heartbeat(force=True)
        self.write_terminal_evidence()
        with (self.run_root / "finalize_v5_terminal.log").open(
            "w", encoding="utf-8"
        ) as stream:
            completed = subprocess.run(
                [
...
def main() -> int:
    ...
    lock_path = (
        ...
        if args.lock_path
        else run_root.parent / ".day5_startup_sync_v5_supervisor.lock"
    )
    ...
    try:
        ...
        if run_root.exists():
            ...
            supervisor.write_terminal_evidence()
            try:
                supervisor.finalize_partial(
