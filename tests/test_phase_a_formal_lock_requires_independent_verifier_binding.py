...
def test_missing_independent_verifier_binding_is_rejected(tmp_path):
    case = make_valid_formal_lock(tmp_path)
    value = copy.deepcopy(case.value)
    del value["implementation_bindings"]["independent_verifier_sha256"]
    rewrite_payload_sha(value)
    path = tmp_path / "missing-verifier.json"
