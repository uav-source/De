...
def test_resume_rejects_wrong_implementation_sha(tmp_path):
    value = sample_result()
    ...
    expected["implementation_sha256"] = "b" * 64
    with pytest.raises(CorruptExistingResult):
        validate_existing_trial_result_for_resume(path, manifest_entry=entry, expected=expected)
