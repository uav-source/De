...
def _pair() -> tuple[list[dict[str, object]], list[dict[str, object]]]:
    ...
    for scan in range(135, 161):
        row: dict[str, object] = {field: scan + 100 for field in REQUIRED_FIELDS}
        for field in row:
...
def test_day7_is_never_authorized_by_classifier() -> None:
    first, second = _pair()
    second[1]["formal_correspondence_checksum"] = 999
    result = compare_pair(first, second)
    assert result["day7_recommended"] is True
    assert result["day7_authorized"] is False


def test_no_divergence_recommends_but_does_not_authorize_more_replay() -> None:
    first, second = _pair()
    result = compare_pair(first, second)
    assert result["additional_replay_recommended"] is True
    assert result["additional_replay_authorized"] is False
    assert result["experiment_a_pass"] is False


def test_evidence_gap_fails_execution_comparison_gate() -> None:
    first, second = _pair()
