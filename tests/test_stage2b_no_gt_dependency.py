...
def test_nonoracle_estimator_ignores_gt_fields():
    observations = simple_observations()
    with_gt = dict(observations, pose_gt=np.ones((4, 8)), axis_per_frame=np.ones((4, 3)))
    first = run_map_lio(with_gt, simple_motion(), detector_config(), update_config(), "huber_full", 0.2, 0.5)
    second = run_map_lio(observations, simple_motion(), detector_config(), update_config(), "huber_full", 0.2, 0.5)
