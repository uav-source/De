"""Paired Development/Test pipeline for Weak-Subspace Update Stage 2C."""
...
from eval import weak_update_common as update_common
from eval.weak_update_stage2c_lock import (
    STAGE2A_LOCK,
    build_update_lock,
    stage2c_config_paths,
    stage2c_source_paths,
...
METHODS = [
    "huber_full",
    "huber_global_gain",
    "huber_directional_mode",
    "huber_projected_gain",
    "huber_oracle_projected_gain",
]
...
def run_weak_update_stage2c(
    ...
    if phase not in {"quick", "development", "test"}:
        raise ValueError(f"unsupported Stage 2C phase: {phase}")
    if int(workers) < 1:
        raise ValueError("workers must be at least one")
    root = Path(root).resolve()
    common = load_yaml(root / "configs/update/stage2c_common.yaml")
    phase_config = normalize_phase_config(load_yaml(root / f"configs/update/stage2c_{phase}.yaml"))
    development = normalize_phase_config(load_yaml(root / "configs/update/stage2c_development.yaml"))
    reserved_test = normalize_phase_config(load_yaml(root / "configs/update/stage2c_test.yaml"))
    ...
    if phase == "test":
        ...
        provenance_path = root / "results/weak_update_stage2c/pytest_provenance.json"
    ...
    data_run = root / "data/weak_update_stage2c" / phase / run_id
    result_run = root / "results/weak_update_stage2c" / phase / run_id
    ...
    if len(blocks) != int(phase_config["expected_sensor_stress_blocks"]):
        raise RuntimeError("Stage 2C sensor-stress block count is incomplete")
    ...
    if phase == "test":
        ...
        development_go = selected_alpha is not None
        write_csv(
            result_run / "tables/development_go_summary.csv",
            [{
                "phase": phase,
                "development_go": development_go,
                "selected_attenuation_alpha": selected_alpha,
                "selection_rule": (
                    "maximize minimum geometry/observation severe-axis reduction; "
                    "within 1 percentage point choose largest alpha"
                ),
            }],
        )
        if selected_alpha is None:
            if phase == "quick":
                # Quick is a smoke test, not a tuning phase.  Use the weakest
                # intervention only to exercise all five formal code paths.
                selected_alpha = max(float(value) for value in common["attenuation_alpha_candidates"])
                for row in alpha_rows:
                    row["quick_engineering_fallback"] = True
                    row["selected"] = float(row["attenuation_alpha"]) == selected_alpha
                write_csv(result_run / "tables/alpha_selection.csv", alpha_rows)
            else:
                ...
                manifest["development_go"] = False
                manifest["alpha_selection"] = alpha_rows
                _write_no_go_outputs(
                    result_run,
                    threshold,
                    alpha_rows,
                    len(blocks),
    ...
    if len(trial_rows) != expected_trials:
        raise RuntimeError(f"Stage 2C method trial count {len(trial_rows)} != {expected_trials}")
    ...
    write_csv(result_run / "tables/formulation_audit.csv", formulation_audit(trial_rows))
    ...
    authorizations = None
    if phase == "test":
        gates = evaluate_stage2c_gates(
            trial_rows, analyses, pairing_audit, stress_audit, common, lock_checks or {}, provenance
        )
        authorizations = {
            **stage2c_authorizations(gates),
        }
        write_gate_outputs(result_run, gates, authorizations, analyses)
    else:
        gates = evaluate_pretest_gates(
            phase, trial_rows, pairing_audit, stress_audit, common,
        )
        authorizations = stage2c_authorizations(gates)
        write_csv(
            result_run / "tables/gate_summary.csv",
            [
                *({"gate": key, "status": value} for key, value in gates.items() if key != "checks"),
                *({"gate": key, "status": value} for key, value in authorizations.items()),
            ],
        )
    ...
    manifest["alpha_selection"] = alpha_rows
    manifest["development_go"] = True if phase in {"development", "test"} else None
...
def evaluate_pretest_gates(
    phase: str,
    rows: Sequence[Mapping[str, Any]],
    pairing_audit: Sequence[Mapping[str, Any]],
    stress_audit: Sequence[Mapping[str, Any]],
    ...
    formulation_rows = formulation_audit(rows)
    engineering_pass = bool(rows and pairing_violations == 0 and solver_failures == 0)
    formulation_pass = bool(
        engineering_tests_present()
        and formulation_rows
        and all(float(row["minimum_joseph_eigenvalue"]) >= -1.0e-10 for row in formulation_rows)
    )
    coherent_downweighted = [
        float(row["contaminated_huber_downweighted_ratio"])
        for row in rows
        if row["method"] == "huber_full"
        and row["stress"] == "coherent_subhuber_slip"
        and float(row["contaminated_measurement_ratio"]) > 0.0
    ]
    stress_pass = bool(
        stress_audit
        and all(bool(row["stress_mechanism_valid"]) for row in stress_audit)
        and coherent_downweighted
        and float(np.median(coherent_downweighted))
        <= float(common["gates"]["coherent_huber_downweighted_ratio_max"])
    )
    return {
        "ENGINEERING": "ENGINEERING_PASS" if engineering_pass else "ENGINEERING_FAIL",
        "FORMULATION": "FORMULATION_PASS" if formulation_pass else "FORMULATION_FAIL",
        "STRESS_MECHANISM": "STRESS_MECHANISM_PASS" if stress_pass else "STRESS_MECHANISM_FAIL",
        "DEVELOPMENT": "DEVELOPMENT_PASS" if phase == "development" else "QUICK_SMOKE_ONLY",
        "GEOMETRY_COHERENT_STRESS": "NOT_EVALUATED_RESERVED_TEST_ONLY",
        "OBSERVATION_COHERENT_STRESS": "NOT_EVALUATED_RESERVED_TEST_ONLY",
        "CLEAN_NONINFERIORITY": "NOT_EVALUATED_RESERVED_TEST_ONLY",
        "OPEN_CONTROL_SAFETY": "NOT_EVALUATED_RESERVED_TEST_ONLY",
        "PROJECTED_GAIN_UPDATE": "PROJECTED_GAIN_UPDATE_FAIL",
        "checks": {
            "pairing_violations": pairing_violations,
            "solver_failures": solver_failures,
            "coherent_huber_downweighted_ratio_median": (
                float(np.median(coherent_downweighted)) if coherent_downweighted else float("nan")
            ),
        },
    }
...
def normalize_phase_config(config: Mapping[str, Any]) -> Dict[str, Any]:
    return update_common.normalize_phase_config(config)


def prepare_run_directories(data_run: Path, result_run: Path, resume: bool, overwrite: bool) -> None:
    update_common.prepare_run_directories(data_run, result_run, resume, overwrite)


def build_stage2c_specs(common: Mapping[str, Any], geometry_seed: int, phase: str) -> List[Dict[str, Any]]:
    return update_common.build_experiment_specs(common, geometry_seed, phase, "stage2c")
...
def prepare_blocks(
    ...
    for geometry_seed in phase_config["geometry_seeds"]:
        for spec in build_stage2c_specs(common, geometry_seed, phase):
            ...
            for sensor_seed in phase_config["sensor_seeds"]:
                ...
                for stress_name in phase_config["stress_names"]:
                    stressed = apply_correspondence_stress(
                        observations, stress_name, stress_config, "weak_update_stage2c",
...
def observation_patch_ids(sequence_dir: Path, observations: Mapping[str, np.ndarray]) -> np.ndarray:
    return update_common.observation_patch_ids(sequence_dir, observations)
...
def load_stressed_block(
    ...
    stressed = apply_correspondence_stress(
        observations, str(block["stress"]), stress_config, "weak_update_stage2c",
...
def make_motion(
    ...
    return simulate_motion_measurements(
        ...
        experiment_family="weak_update_stage2c", geometry_seed=geometry_seed,
        sensor_seed=sensor_seed,
    )
...
def map_paired_blocks(blocks: Sequence[Mapping[str, Any]], function, workers: int):
    return update_common.map_paired_blocks(blocks, function, workers)
...
def _run_development_block(
    ...
    for process_seed in phase_config["process_seeds"]:
        ...
        local_base.append(run_method_trial(
            block, observations, motion, process_seed, "huber_full", threshold,
            1.0, detector_config, common,
        ))
        for alpha in common["attenuation_alpha_candidates"]:
            row = run_method_trial(
                block, observations, motion, process_seed, "huber_projected_gain",
...
def run_method_trial(
    ...
    oracle = np.asarray(observations["axis_per_frame"], dtype=float) if method == "huber_oracle_projected_gain" else None
    output = run_map_lio(
...
def select_attenuation_alpha(
    ...
    for alpha in common["attenuation_alpha_candidates"]:
        ...
        severe_by_sweep = {
            sweep: [
                pair
                for pair in pairs
                if pair[0]["sweep"] == sweep
                and pair[0]["stress"] == "coherent_subhuber_slip"
                and pair[0]["level"] in SEVERE
            ]
            for sweep in ("geometry", "observation")
        }
        row: Dict[str, Any] = {"attenuation_alpha": float(alpha)}
        for prefix, selected in [("open_control", oc), ("clean", clean)]:
            for metric in METRICS:
                row[f"{prefix}_{metric}_relative_change"] = median_relative_change(selected, metric)
        for sweep, selected in severe_by_sweep.items():
            reductions = [
                -relative_change(float(candidate["axis_rmse"]), float(baseline["axis_rmse"]))
                for candidate, baseline in selected
            ]
            by_geometry: Dict[int, List[float]] = defaultdict(list)
            for (candidate, _), reduction in zip(selected, reductions):
                by_geometry[int(candidate["geometry_seed"])].append(reduction)
            geometry_effects = [float(np.median(values)) for values in by_geometry.values()]
            row[f"{sweep}_severe_axis_reduction"] = (
                float(np.median(reductions)) if reductions else float("nan")
            )
            row[f"{sweep}_positive_ratio"] = (
                float(np.mean(np.asarray(geometry_effects) > 0.0))
                if geometry_effects else float("nan")
            )
            for metric in METRICS[1:]:
                row[f"{sweep}_severe_{metric}_relative_change"] = median_relative_change(
                    selected, metric
                )
        row["geometry_severe_axis_reduction"] = row["geometry_severe_axis_reduction"]
        row["observation_severe_axis_reduction"] = row["observation_severe_axis_reduction"]
        row["minimum_sweep_axis_reduction"] = min(
            float(row["geometry_severe_axis_reduction"]),
            float(row["observation_severe_axis_reduction"]),
        )
        ...
        row["feasible"] = bool(
            ...
            and row["clean_trajectory_rmse_3d_relative_change"] <= 0.03
            and row["geometry_severe_axis_reduction"] > 0.0
            and row["observation_severe_axis_reduction"] > 0.0
            and row["geometry_positive_ratio"] >= 0.60
            and row["observation_positive_ratio"] >= 0.60
            and row["geometry_severe_strong_translation_rmse_relative_change"] <= 0.03
            and row["geometry_severe_orientation_rmse_rad_relative_change"] <= 0.03
            and row["observation_severe_strong_translation_rmse_relative_change"] <= 0.03
            and row["observation_severe_orientation_rmse_rad_relative_change"] <= 0.03
        )
        row["selected"] = False
        table.append(row)
    ...
    if not feasible:
        return table, None
    best = max(float(row["minimum_sweep_axis_reduction"]) for row in feasible)
    selected = max(
        float(row["attenuation_alpha"])
        for row in feasible if float(row["minimum_sweep_axis_reduction"]) >= best - 0.01
    )
    for row in table:
...
def aggregate_method_trials(rows: Sequence[Mapping[str, Any]]) -> List[Dict[str, Any]]:
    return update_common.aggregate_method_trials(rows, METRICS)
...
def analyze_trial_results(rows: Sequence[Mapping[str, Any]], common: Mapping[str, Any]) -> Dict[str, Any]:
    ...
    for sweep in ["geometry", "observation"]:
        selected = [row for row in rows if row["sweep"] == sweep and row["stress"] == "coherent_subhuber_slip" and row["level"] in SEVERE and row["method"] == "huber_projected_gain"]
        effects = []
        ...
        for row in selected:
            key = trial_key(row)
            full = indexed[key + ("huber_full",)]
            global_row = indexed[key + ("huber_global_gain",)]
            ...
            global_orientation.append(-relative_change(
                float(row["orientation_rmse_rad"]),
                float(global_row["orientation_rmse_rad"]),
            ))
        ...
        contaminated.append({
            ...
            "projected_vs_global_axis_advantage": float(np.median(global_axis)),
            "projected_vs_global_strong_advantage": float(np.median(global_strong)),
            "projected_vs_global_orientation_advantage": float(np.median(global_orientation)),
        })
    ...
    projected_vs_global = comparison_between_methods(
        rows, indexed, "huber_projected_gain", "huber_global_gain"
    )
    projected_vs_directional = comparison_between_methods(
        rows, indexed, "huber_projected_gain", "huber_directional_mode"
    )
    return {
        "tables": {
            "contaminated_severe_results": contaminated,
            "coherent_stress_results": contaminated,
            "clean_noninferiority": clean,
            "clean_noninferiority_by_level": clean_by_level,
            "open_control_safety": open_control,
            "oracle_gap": oracle,
            "oracle_gap_full_coverage": oracle,
            "projected_vs_global": projected_vs_global,
            "projected_vs_directional_mode": projected_vs_directional,
            "per_geometry_paired_effects": per_geometry_effects,
        }
    }


def comparison_between_methods(
    rows: Sequence[Mapping[str, Any]],
    ...
    selected = [
        ...
        and row["stress"] == "coherent_subhuber_slip"
        and row["level"] in SEVERE
    ]
    for sweep in ("geometry", "observation"):
        values = [row for row in selected if row["sweep"] == sweep]
        record: Dict[str, Any] = {
            "sweep": sweep,
        ...
        for metric in METRICS:
            record[f"{metric}_relative_change"] = float(np.median([
                relative_change(
                    float(row[metric]),
                    float(indexed[trial_key(row) + (baseline_method,)][metric]),
                )
                for row in values
...
def comparison_scope(
    ...
    selective = [row for row in rows if row["method"] == "huber_projected_gain" and predicate(row)]
...
def oracle_gap(rows: Sequence[Mapping[str, Any]], indexed: Mapping[tuple, Mapping[str, Any]]) -> List[Dict[str, Any]]:
    ...
    rmse_gaps = []
    for row in rows:
        if row["method"] != "huber_projected_gain" or row["stress"] != "coherent_subhuber_slip" or row["level"] not in SEVERE:
            continue
        key = trial_key(row)
        full = indexed[key + ("huber_full",)]
        oracle = indexed[key + ("huber_oracle_projected_gain",)]
        gain_selective = float(full["axis_rmse"]) - float(row["axis_rmse"])
        gain_oracle = float(full["axis_rmse"]) - float(oracle["axis_rmse"])
        selective_gains.append(gain_selective)
        oracle_gains.append(gain_oracle)
        rmse_gaps.append(float(row["axis_rmse"]) - float(oracle["axis_rmse"]))
    eligible = [
        selective / oracle
        for selective, oracle in zip(selective_gains, oracle_gains)
        if oracle > 0.0
    ]
    total = len(oracle_gains)
    oracle_positive = sum(value > 0.0 for value in oracle_gains)
    selective_positive = sum(value > 0.0 for value in selective_gains)
    both_positive = sum(
        selective > 0.0 and oracle > 0.0
        for selective, oracle in zip(selective_gains, oracle_gains)
    )
    return [{
        "total_pair_count": total,
        "oracle_positive_pair_count": oracle_positive,
        "oracle_nonpositive_pair_count": total - oracle_positive,
        "oracle_positive_fraction": oracle_positive / total if total else float("nan"),
        "selective_positive_pair_count": selective_positive,
        "selective_positive_fraction": selective_positive / total if total else float("nan"),
        "both_positive_pair_count": both_positive,
        "median_selective_gain_all_pairs": (
            float(np.median(selective_gains)) if selective_gains else float("nan")
        ),
        "median_oracle_gain_all_pairs": (
            float(np.median(oracle_gains)) if oracle_gains else float("nan")
        ),
        "median_selective_minus_oracle_rmse_all_pairs": (
            float(np.median(rmse_gaps)) if rmse_gaps else float("nan")
        ),
        "conditional_gain_capture_median": (
            float(np.median(eligible)) if eligible else float("nan")
        ),
        "conditional_eligible_pair_count": len(eligible),
        "conditional_coverage_fraction": len(eligible) / total if total else float("nan"),
    }]


def formulation_audit(rows: Sequence[Mapping[str, Any]]) -> List[Dict[str, Any]]:
    output = []
    for method in METHODS:
        values = [row for row in rows if row["method"] == method]
        actionable = [row for row in values if float(row["actionable_rate"]) > 0.0]
        weak_errors = [
            abs(
                float(row["mean_actionable_weak_correction_ratio"])
                - float(row["attenuation_alpha"])
            )
            for row in actionable
        ] if method in {"huber_projected_gain", "huber_oracle_projected_gain"} else []
        output.append({
            "method": method,
            "trial_count": len(values),
            "solver_failure_count": sum(int(row["solver_failure"]) for row in values),
            "minimum_joseph_eigenvalue": min(
                (float(row["minimum_joseph_eigenvalue"]) for row in values),
                default=float("nan"),
            ),
            "maximum_mean_weak_ratio_error": max(weak_errors, default=0.0),
            "maximum_mean_strong_difference": max(
                (float(row["mean_strong_correction_difference_norm"]) for row in values),
                default=0.0,
            ),
            "maximum_mean_rotation_difference": max(
                (float(row["mean_rotation_correction_difference_norm"]) for row in values),
                default=0.0,
            ),
        })
    return output
...
def _write_no_go_outputs(
    ...
    for name in [
        ...
        "sensor_stress_method_summary",
        "paired_method_differences",
        "coherent_stress_results",
        "clean_noninferiority",
        "open_control_safety",
        "projected_vs_global",
        "projected_vs_directional_mode",
        "oracle_gap_full_coverage",
        "pairing_audit",
    ]:
        write_csv(result_run / f"tables/{name}.csv", [])
    write_csv(result_run / "tables/stress_audit.csv", stress_audit)
    candidate_formulation = formulation_audit([*base_rows, *candidate_rows])
    write_csv(result_run / "tables/formulation_audit.csv", candidate_formulation)
    coherent_downweighted = [
        float(row["contaminated_huber_downweighted_ratio"])
    ...
    gates = {
        "ENGINEERING": "ENGINEERING_PASS" if not any(int(row["solver_failure"]) for row in [*base_rows, *candidate_rows]) else "ENGINEERING_FAIL",
        "FORMULATION": "FORMULATION_PASS" if engineering_tests_present() else "FORMULATION_FAIL",
        "STRESS_MECHANISM": "STRESS_MECHANISM_PASS" if stress_pass else "STRESS_MECHANISM_FAIL",
        "DEVELOPMENT": "DEVELOPMENT_NO_GO",
        "GEOMETRY_COHERENT_STRESS": "NOT_RUN_DEVELOPMENT_NO_GO",
        "OBSERVATION_COHERENT_STRESS": "NOT_RUN_DEVELOPMENT_NO_GO",
        "CLEAN_NONINFERIORITY": "NOT_RUN_DEVELOPMENT_NO_GO",
        "OPEN_CONTROL_SAFETY": "NOT_RUN_DEVELOPMENT_NO_GO",
        "PROJECTED_GAIN_UPDATE": "PROJECTED_GAIN_UPDATE_FAIL",
    }
    authorizations = stage2c_authorizations(gates)
    write_csv(
        result_run / "tables/gate_summary.csv",
        [
            *({"gate": key, "status": value} for key, value in gates.items()),
            *({"gate": key, "status": value} for key, value in authorizations.items()),
        ],
    )
    (result_run / "reports/development_report.md").write_text(
        "# Stage 2C Development Report\n\n"
        f"- Sensor-stress blocks: {block_count}\n"
        "- DEVELOPMENT_PASS: **NO_GO**\n"
        "- Selected attenuation alpha: none\n"
        f"- Motion-prior Open Control ODI threshold: {threshold['online_odi_threshold']}\n\n"
        "No alpha improved both severe geometry and observation sweeps. "
        "No update lock was created and reserved Test seeds remain unconsumed.\n",
        encoding="utf-8",
    )


def evaluate_stage2c_gates(
    ...
    for row in contaminated_rows:
        contaminated_checks[row["sweep"]] = bool(
            ...
            and float(row["projected_vs_global_axis_advantage"]) >= -float(
                gates["projected_vs_global_axis_worse_max"]
            )
            and (
                float(row["projected_vs_global_strong_advantage"])
                >= float(gates["projected_vs_global_protection_advantage_min"])
                or float(row["projected_vs_global_orientation_advantage"])
                >= float(gates["projected_vs_global_protection_advantage_min"])
            )
        )
    ...
    stress_pass = bool(
        ...
        and coherent_downweighted
        and float(np.median(coherent_downweighted))
        <= float(gates["coherent_huber_downweighted_ratio_max"])
    )
    formulation_rows = formulation_audit(rows)
    formulation_pass = bool(
        formulation_rows
        and all(int(row["solver_failure_count"]) == 0 for row in formulation_rows)
        and all(float(row["minimum_joseph_eigenvalue"]) >= -1.0e-10 for row in formulation_rows)
        and engineering_tests_present()
    )
    projected_pass = bool(
        engineering_pass and formulation_pass and stress_pass and contaminated_checks.get("geometry", False)
    ...
    return {
        ...
        "GEOMETRY_COHERENT_STRESS": (
            "GEOMETRY_COHERENT_STRESS_PASS"
            if contaminated_checks.get("geometry", False)
            else "GEOMETRY_COHERENT_STRESS_FAIL"
        ),
        "OBSERVATION_COHERENT_STRESS": (
            "OBSERVATION_COHERENT_STRESS_PASS"
            if contaminated_checks.get("observation", False)
            else "OBSERVATION_COHERENT_STRESS_FAIL"
        ),
        "CLEAN_NONINFERIORITY": (
            "CLEAN_NONINFERIORITY_PASS" if clean_pass else "CLEAN_NONINFERIORITY_FAIL"
        ),
        "OPEN_CONTROL_SAFETY": (
            "OPEN_CONTROL_SAFETY_PASS" if control_pass else "OPEN_CONTROL_SAFETY_FAIL"
        ),
        "PROJECTED_GAIN_UPDATE": (
            "PROJECTED_GAIN_UPDATE_PASS" if projected_pass else "PROJECTED_GAIN_UPDATE_FAIL"
        ),
        "checks": {
            "pairing_violations": pairing_violations,
            "solver_failures": solver_failures,
...
def stage2c_authorizations(gates: Mapping[str, Any]) -> Dict[str, bool]:
    """Map the complete gate result to the only allowed downstream decisions."""

    passed = bool(gates.get("PROJECTED_GAIN_UPDATE") == "PROJECTED_GAIN_UPDATE_PASS")
    return {
        "PROJECTED_GAIN_UPDATE_PASS": passed,
...
def lock_update_analysis(root: Path, development_run_dir: Path) -> Dict[str, Any]:
    ...
    if (root / "results/weak_update_stage2c/development").resolve() not in run_dir.parents:
        raise ValueError("--lock-update accepts only a Stage 2C development run")
    ...
    common = load_yaml(root / "configs/update/stage2c_common.yaml")
    development = normalize_phase_config(load_yaml(root / "configs/update/stage2c_development.yaml"))
    reserved = normalize_phase_config(load_yaml(root / "configs/update/stage2c_test.yaml"))
    lock = build_update_lock(
        ...
        common, load_yaml(root / "configs/toy_lio/motion_surrogate_stage2c.yaml"),
        load_yaml(root / "configs/update/stage2c_stress.yaml"), development, reserved,
    )
    write_json(output, lock)
    frozen = root / "artifacts/current/weak_update_stage2c/locked/update_lock.json"
    frozen.parent.mkdir(parents=True, exist_ok=True)
    write_json(frozen, lock)
    return lock


def analyze_existing_stage2c(root: Path, run_dir: Path) -> Dict[str, Any]:
...
def build_manifest(
    ...
    return {
        "stage": "weak_subspace_update_stage2c", "phase": phase, "run_id": run_id,
        ...
        "source_tree_sha256": compute_source_tree_hash(stage2c_source_paths(root)),
        "config_bundle_sha256": compute_bundle_hash(stage2c_config_paths(root)),
...
def write_gate_outputs(result_run: Path, gates: Mapping[str, Any], authorizations: Mapping[str, bool], analyses: Mapping[str, Any]) -> None:
    ...
    lines = ["# Weak-Subspace Update Stage 2C Gate Report", ""]
    ...
    lines.extend(["", "Oracle is an offline diagnostic only. Stage 2C does not integrate FAST-LIO2, use real IMU propagation, implement real data association, or reopen risk prediction.", ""])
    (result_run / "reports/weak_update_stage2c_gate_report.md").write_text("\n".join(lines), encoding="utf-8")
    (result_run / "reports/weak_update_stage2c_reproducibility_report.md").write_text(
        "# Stage 2C Reproducibility\n\nTest used a committed update lock, disjoint seeds, shared observations/process noise/stress, and frozen alpha.\n",
        encoding="utf-8",
    )
...
def export_current_artifacts(root: Path, result_run: Path, lock_path: Path, manifest: Mapping[str, Any]) -> None:
    target = root / "artifacts/current/weak_update_stage2c"
    target.mkdir(parents=True, exist_ok=True)
    shutil.copy2(lock_path, target / "update_lock.json")
    for relative_path in [
        "reports/weak_update_stage2c_gate_report.md", "reports/weak_update_stage2c_reproducibility_report.md",
...
def generate_figures(result_run: Path, rows: Sequence[Mapping[str, Any]], alpha_rows: Sequence[Mapping[str, Any]]) -> None:
    ...
    figure_names = [
        ...
        "projected_vs_global_paired", "projected_vs_huber_paired", "oracle_gap",
    ...
    for index, name in enumerate(figure_names):
        ...
        if name == "alpha_development_tradeoff" and alpha_rows:
            axis.plot(
                [float(row["attenuation_alpha"]) for row in alpha_rows],
                [float(row["minimum_sweep_axis_reduction"]) for row in alpha_rows],
                marker="o",
            )
            axis.set_xlabel("attenuation alpha")
            axis.set_ylabel("median axis RMSE reduction")
        else:
            selected = [row for row in rows if row["method"] in {"huber_full", "huber_projected_gain", "huber_global_gain", "huber_oracle_projected_gain"}][:400]
            x = np.arange(len(selected))
            y = [float(row["axis_rmse"] if "axis" in name or "oracle" in name else row["strong_translation_rmse"]) for row in selected]
            colors = ["tab:purple" if row["method"] == "huber_oracle_projected_gain" else "tab:blue" for row in selected]
...
def stress_mechanism_audit(
    ...
    selected = np.asarray(stressed["selected_axial_patch_ids"]).astype(str).tolist()
    start = int(np.asarray(stressed["shared_burst_start_frame"]).item())
    sign = int(np.asarray(stressed["shared_burst_sign"]).item())
    injected_sigma = float(np.asarray(stressed["injected_slip_sigma"]).item())
    configured_duration = (
    ...
    duration_valid = bool(
        ...
        or (
            start >= 0
            and start + configured_duration <= mask.shape[0]
            and sign in {-1, 1}
        )
    )
    ...
    offsets = np.asarray(stressed["contamination_offset_m"], dtype=float)[mask]
    shared_sign_valid = bool(not offsets.size or np.all(np.sign(offsets) == sign))
    configured_sigma = (
        0.0 if stress_name == "clean"
        else float(stress_config["stress_regimes"][stress_name]["slip_sigma"])
    )
    slip_sigma_valid = bool(
        not np.any(axial)
        or np.isclose(injected_sigma, configured_sigma, atol=1.0e-12)
    )
    subhuber_valid = bool(
        stress_name != "coherent_subhuber_slip"
        or injected_sigma < 2.5
    )
    valid = bool(
        ...
        and locked_anchor_delta and shared_sign_valid and slip_sigma_valid and subhuber_valid
    )
    return {
        ...
        "shared_burst_start_frame": start,
        "shared_burst_sign": sign,
        "shared_sign_valid": shared_sign_valid,
        "nominal_measurement_sigma_m": float(
            np.asarray(stressed["nominal_measurement_sigma_m"]).item()
        ),
        "injected_slip_distance_m": float(
            np.asarray(stressed["injected_slip_distance_m"]).item()
        ),
        "injected_slip_sigma": injected_sigma,
        "slip_sigma_valid": slip_sigma_valid,
        "subhuber_valid": subhuber_valid,
        "selected_axial_patch_ids": "|".join(selected),
        "contaminated_measurement_ratio": float(
            np.asarray(stressed["contaminated_measurement_ratio"]).item()
        ),
...
def observation_checksum(observations: Mapping[str, np.ndarray]) -> str:
    return update_common.observation_checksum(observations)


def sweep_name(spec: Mapping[str, Any]) -> str:
    return update_common.sweep_name(spec)


def trial_key(row: Mapping[str, Any]) -> tuple:
    return update_common.trial_key(row)


def index_rows(rows: Sequence[Mapping[str, Any]], include_method: bool = False) -> Dict[tuple, Mapping[str, Any]]:
    return update_common.index_rows(rows, include_method)


def relative_change(candidate: float, baseline: float) -> float:
    return update_common.relative_change(candidate, baseline)


def median_relative_change(pairs: Sequence[tuple[Mapping[str, Any], Mapping[str, Any]]], metric: str) -> float:
    return update_common.median_relative_change(pairs, metric)


def block_bootstrap_effect(effects: Sequence[Mapping[str, Any]], repetitions: int, seed: int) -> Dict[str, Any]:
    return update_common.block_bootstrap_effect(effects, repetitions, seed)


def engineering_tests_present() -> bool:
    root = Path(__file__).resolve().parents[2]
    required = [
        "tests/test_prior_relinearization.py",
        "tests/test_stage2c_no_gt_dependency.py",
        "tests/test_covariance_propagation.py",
        "tests/test_projected_gain_joseph_psd.py",
        "tests/test_projected_gain_alpha_one_equivalence.py",
        "tests/test_projected_gain_nontrigger_equivalence.py",
        "tests/test_directional_mode_psd.py",
    ]
...
def analysis_record(root: Path) -> Dict[str, Any]:
    return {
        ...
        "analysis_source_tree_sha256": compute_source_tree_hash(stage2c_source_paths(root)),
    }
