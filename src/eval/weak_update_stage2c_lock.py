"""Immutable source/config/update lock for Weak-Subspace Update Stage 2C."""
...
STAGE2A_ARTIFACT = "artifacts/current/detector_stage2a"
STAGE2B_NO_GO_ARTIFACT = "artifacts/history/stage2b_column_scaling_no_go"


def stage2c_config_paths(root: Path) -> list[Path]:
    root = Path(root).resolve()
    return [
        root / "configs/update/stage2c_common.yaml",
        root / "configs/update/stage2c_quick.yaml",
        root / "configs/update/stage2c_development.yaml",
        root / "configs/update/stage2c_test.yaml",
        root / "configs/update/stage2c_stress.yaml",
        root / "configs/toy_lio/motion_surrogate_stage2c.yaml",
        root / "configs/detector/odi_stage2a.yaml",
    ]


def stage2c_source_paths(root: Path) -> list[Path]:
    root = Path(root).resolve()
    return [
        ...
        root / "src/eval/weak_update_stage2c.py",
        root / "src/eval/weak_update_stage2c_lock.py",
        root / "src/eval/weak_update_common.py",
        root / "src/eval/synthetic_pipeline_common.py",
        root / "src/eval/hierarchical_statistics.py",
        root / "scripts/32_run_weak_update_stage2c.py",
        root / "scripts/29_run_verified_pytest.py",
        root / "tests",
        *stage2c_config_paths(root),
        root / "pytest.ini",
    ]
...
def build_update_lock(
    ...
    root = Path(root).resolve()
    source_hash, snapshot = compute_source_tree_snapshot(stage2c_source_paths(root))
    return {
        "stage2a_detector_artifact_sha256": compute_directory_hash(root / STAGE2A_ARTIFACT),
        "stage2b_no_go_artifact_sha256": compute_directory_hash(root / STAGE2B_NO_GO_ARTIFACT),
        "detector_metric_version": str(common["metric_definition_version"]),
        "update_definition_version": str(common["update_definition_version"]),
        ...
        "alpha_selection_rule": (
            "feasible positive geometry and observation reductions; maximize minimum sweep "
            "axis reduction; ties within 1 percentage point choose largest alpha"
        ),
        "development_go": True,
        "initial_covariance_diag": list(common["initial_covariance_diag"]),
        "motion_covariance_parameters": dict(motion),
        "main_stress_parameters": dict(stress["stress_regimes"]["coherent_subhuber_slip"]),
        "diagnostic_stress_parameters": dict(stress["stress_regimes"]["gross_outlier_control"]),
        ...
        "global_comparator": str(common["global_comparator"]),
        "mechanistic_ablation": str(common["mechanistic_ablation"]),
        ...
        "statistics_code_sha256": compute_source_tree_hash([
            root / "src/eval/update_metrics.py", root / "src/eval/weak_update_stage2c.py",
        ]),
...
def verify_update_lock(root: Path, lock: Mapping[str, Any], require_clean: bool = True) -> Dict[str, Any]:
    root = Path(root).resolve()
    source_hash, snapshot = compute_source_tree_snapshot(stage2c_source_paths(root))
    config_hash = compute_bundle_hash(stage2c_config_paths(root))
    stage2a_hash = compute_directory_hash(root / STAGE2A_ARTIFACT)
    stage2b_hash = compute_directory_hash(root / STAGE2B_NO_GO_ARTIFACT)
    changed = changed_snapshot_files(lock.get("source_file_hashes", {}), snapshot)
    common = _load_yaml(root / "configs/update/stage2c_common.yaml")
    stress = _load_yaml(root / "configs/update/stage2c_stress.yaml")
    development = _load_yaml(root / "configs/update/stage2c_development.yaml")
    reserved = _load_yaml(root / "configs/update/stage2c_test.yaml")
    ...
    checks = {
        ...
        "development_go": lock.get("development_go") is True,
        "update_definition_matched": (
            str(lock.get("update_definition_version")) == str(common["update_definition_version"])
        ),
        "stage2a_detector_artifact_matched": (
            stage2a_hash == str(lock.get("stage2a_detector_artifact_sha256"))
        ),
        "stage2b_no_go_artifact_matched": (
            stage2b_hash == str(lock.get("stage2b_no_go_artifact_sha256"))
        ),
...
def _load_yaml(path: Path) -> Dict[str, Any]:
    ...
    if not isinstance(value, dict):
        raise ValueError(f"Stage 2C lock input must be a mapping: {path}")
